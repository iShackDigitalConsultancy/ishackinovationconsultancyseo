      ...
      <div class="sm:mx-auto sm:w-full sm:max-w-md z-10">
        <div class="flex justify-center flex-col items-center">
          ...
          <p class="mt-2 text-center text-sm text-slate-600">
            Or
            <a routerLink="/register" class="font-medium text-primary hover:text-blue-500 transition-colors">
        ...
        <div class="glass-panel py-8 px-4 shadow-2xl sm:rounded-2xl sm:px-10 border border-white/50 bg-white/60 backdrop-blur-xl">
          ...
          <form class="space-y-6" (ngSubmit)="onSubmit()">
            ...
            <div>
              ...
              <div class="mt-1">
                <input id="password" name="password" type="password" autocomplete="current-password" required [(ngModel)]="password" [disabled]="loading" class="appearance-none block w-full px-4 py-3 border border-slate-300 rounded-xl shadow-sm placeholder-slate-400 focus:outline-none focus:ring-primary focus:border-primary sm:text-sm bg-white/70 transition-all disabled:opacity-50">
              </div>
            </div>
